...
test.describe("Create Category Page", () => {
  ...
  test("missing fields should return their error messages", async ({
    page,
  }) => {
    // leave form empty & submit
    await page.getByRole("button", { name: "CREATE PRODUCT" }).click();
    // assert error toast
    await expect(page.getByRole("main")).toContainText(/category is required/i);
    ...
    await page.getByRole("button", { name: "CREATE PRODUCT" }).click();
    // assert error toast
    await expect(page.getByRole("main")).toContainText(/quantity is required/i);
    ...
    await page.getByRole("button", { name: "CREATE PRODUCT" }).click();
    // assert error toast
    await expect(page.getByRole("main")).toContainText(/shipping is required/i);
  });
